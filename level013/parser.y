...
stmt
    ...
    | INT IDENT '[' INTEGER ']' ';' {
        SymTab.declareArray($2, $4, 1, MainFn);
        free($2);
      }
    | INT IDENT '[' INTEGER ']' '[' INTEGER ']' ';' {
        SymTab.declareArray($2, $4, $7, MainFn);
        free($2);
      }
    | IDENT '[' expr ']' '=' expr ';' {
    ...
    | IDENT '[' expr ']' '[' expr ']' '=' expr ';' {
        ...
        if (info && info->cols > 1) {
            llvm::Value* totalIdx = Builder->CreateAdd(
                Builder->CreateMul($3, llvm::ConstantInt::get(*TheContext, llvm::APInt(32, info->cols, false))),
                $6, "totalidx");
            llvm::Value* idx[2] = {
                llvm::ConstantInt::get(*TheContext, llvm::APInt(32, 0, false)),
                totalIdx
